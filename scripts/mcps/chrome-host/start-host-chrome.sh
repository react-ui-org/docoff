...
if [ -f "$ENV_FILE" ]; then
  for env_key in CHROME_DEBUG_PORT CHROME_BIN CHROME_DEBUG_BIND COMPOSE_PROJECT_NAME COMPOSE_START_PORT; do
...
set -- "$@" --new-window "http://localhost:${COMPOSE_START_PORT:-8080}"
