...
The `devcontainer` ships several AI coding assistants:
...
This is useful for verifying the rendered component previews, taking
screenshots, and interactively debugging the running dev server in a real
browser.

The wiring lives in the [`scripts/mcps/chrome-host/`](../scripts/mcps/chrome-host/)
directory and works on macOS and Linux.
...
of the project's setup: `setup.sh` runs
`scripts/mcps/chrome-host/mcp-setup.sh`.
...
through Docker Compose). See [`.env.dist`](../.env.dist) for the available
options.
...
1. **Start the debug browser on the host.** Run this from a normal host terminal
   ...
   browser and its logins are untouched. The command is safe to re-run. It opens
   the dev server (`http://localhost:${COMPOSE_START_PORT:-8080}`) by default —
   start it with `npm start` inside the `devcontainer`.
